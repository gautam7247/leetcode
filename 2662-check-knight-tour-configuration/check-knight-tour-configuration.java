class Solution {
    public boolean helper(int[][] grid, int row, int col, int num) {
        ...
        int i, j; // i aur j ko declare kiya gaya hai
        ...
        // 2 right 1 up (1 up 2 right)
        i = row - 1;
        j = col + 2;
        if(i >= 0 && j < n && grid[i][j] == num + 1)
            return helper(grid, i, j, num + 1);
            
        // 2 right 1 down (1 down 2 right)
        i = row + 1;
        j = col + 2;
        if(i < n && j < n && grid[i][j] == num + 1)
            return helper(grid, i, j, num + 1);
            
        // 2 left 1 up (1 up 2 left)
        i = row - 1;
        j = col - 2;
        if(i >= 0 && j >= 0 && grid[i][j] == num + 1)
            return helper(grid, i, j, num + 1);
            
        // 2 left 1 down (1 down 2 left)
        i = row + 1;
        j = col - 2;
        if(i < n && j >= 0 && grid[i][j] == num + 1)
            return helper(grid, i, j, num + 1);
            
        // Agar inme se koi bhi rasta nahi bacha, toh false return hoga
        return false;
    }
